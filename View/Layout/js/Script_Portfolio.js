...
// Función principal para mostrar el contenido en el modal
...
function openImageFromCarousel(carouselItem) {
  ...
  const modalImageContainer = imageModal.querySelector(
          ...
          <div class="image-wrapper">
            <img src="${currentImg.src}" class="modal-image" alt="Image" 
                 ...
                 style="transform: translateX(${currentTranslateX}px);
                        transition: ${
                          transition ? "transform 0.3s ease" : "none"
                        }">
          </div>
        `;
  ...
  function handleTouchMove(e) {
    ...
    if (
      ...
      (currentIndex === totalImages - 1 && diff < 0)
    ) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!heroSection || !projectsSection) {
    console.error(
      "Error: No se encontraron 'hero-section' o 'projects-section' en el DOM."
    );
  }
  ...
  const handleScroll = () => {
    ...
    if (scrollPosition >= windowHeight) {
      ...
      projectsSection.style.transform = `translateY(${
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function openImageFromBentoItem(bentoItem) {
    ...
    currentImages = Array.from(
      currentBentogrid.querySelectorAll(".bento-item img")
    ).map((img) => img.src);
