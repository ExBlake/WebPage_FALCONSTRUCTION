<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Instagram Clone</title>
    <link rel="stylesheet" href="Estilos_Header">
    <link rel="stylesheet" href="Estilos_Resenas">
    <!-- Font Awesome para los iconos -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.2/css/all.min.css">
    <link rel="stylesheet" href="Estilos_Footer">
</head>
<body>
    <?php
    // Inclusión del Header
    require_once 'Layout/Header.php';
    ?>

    <!-- Main Content -->
    <main>
        <div class="container-review">
            <!-- Feed Section -->
            <div class="feed-section-review">
                <!-- Stories -->
                

                <!-- Posts -->
                <div class="posts-review">
                    <div class="post-review">
                        <div class="post-header-review">
                            <img src="/placeholder-user.jpg" alt="user" class="profile-pic-review">
                            <span class="username-review">amethyst_grl</span>
                            <i class="fas fa-ellipsis-h"></i>
                        </div>
                        <div class="post-carousel-review">
                            <div class="carousel-container-review">
                                <div class="carousel-slide-review">
                                    <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-5say60G188V3B6Cy7IiwXtz08q70oq.png" alt="post">
                                </div>
                                <div class="carousel-slide-review">
                                    <video src="video.mp4" controls></video>
                                </div>
                                <div class="carousel-slide-review">
                                    <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-5say60G188V3B6Cy7IiwXtz08q70oq.png" alt="post">
                                </div>
                            </div>
                            <button class="carousel-button-review prev-review">&lt;</button>
                            <button class="carousel-button-review next-review">&gt;</button>
                        </div>
                        <div class="post-actions-review">
                            <div class="actions-left-review">
                                <i class="far fa-heart"></i>
                                <i class="far fa-comment"></i>
                                <i class="far fa-paper-plane"></i>
                            </div>
                            <i class="far fa-bookmark"></i>
                        </div>
                        <div class="post-info-review">
                            <span class="likes-review">1,234 likes</span>
                            <div class="caption-review">
                                <span class="username-review">amethyst_grl</span>
                                Game day! 🏀
                            </div>
                            <span class="timestamp-review">2 HOURS AGO</span>
                        </div>
                    </div>
                </div>
                <!-- Posts -->
                <div class="posts-review">
                    <div class="post-review">
                        <div class="post-header-review">
                            <img src="/placeholder-user.jpg" alt="user" class="profile-pic-review">
                            <span class="username-review">amethyst_grl</span>
                            <i class="fas fa-ellipsis-h"></i>
                        </div>
                        <div class="post-carousel-review">
                            <div class="carousel-container-review">
                                <div class="carousel-slide-review">
                                    <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-5say60G188V3B6Cy7IiwXtz08q70oq.png" alt="post">
                                </div>
                                <div class="carousel-slide-review">
                                    <video src="video.mp4" controls></video>
                                </div>
                                <div class="carousel-slide-review">
                                    <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-5say60G188V3B6Cy7IiwXtz08q70oq.png" alt="post">
                                </div>
                            </div>
                            <button class="carousel-button-review prev-review">&lt;</button>
                            <button class="carousel-button-review next-review">&gt;</button>
                        </div>
                        <div class="post-actions-review">
                            <div class="actions-left-review">
                                <i class="far fa-heart"></i>
                                <i class="far fa-comment"></i>
                                <i class="far fa-paper-plane"></i>
                            </div>
                            <i class="far fa-bookmark"></i>
                        </div>
                        <div class="post-info-review">
                            <span class="likes-review">1,234 likes</span>
                            <div class="caption-review">
                                <span class="username-review">amethyst_grl</span>
                                Game day! 🏀
                            </div>
                            <span class="timestamp-review">2 HOURS AGO</span>
                        </div>
                    </div>
                </div>
                <!-- Posts -->
                <div class="posts-review">
                    <div class="post-review">
                        <div class="post-header-review">
                            <img src="/placeholder-user.jpg" alt="user" class="profile-pic-review">
                            <span class="username-review">amethyst_grl</span>
                            <i class="fas fa-ellipsis-h"></i>
                        </div>
                        <div class="post-carousel-review">
                            <div class="carousel-container-review">
                                <div class="carousel-slide-review">
                                    <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-5say60G188V3B6Cy7IiwXtz08q70oq.png" alt="post">
                                </div>
                                <div class="carousel-slide-review">
                                    <video src="video.mp4" controls></video>
                                </div>
                                <div class="carousel-slide-review">
                                    <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-5say60G188V3B6Cy7IiwXtz08q70oq.png" alt="post">
                                </div>
                            </div>
                            <button class="carousel-button-review prev-review">&lt;</button>
                            <button class="carousel-button-review next-review">&gt;</button>
                        </div>
                        <div class="post-actions-review">
                            <div class="actions-left-review">
                                <i class="far fa-heart"></i>
                                <i class="far fa-comment"></i>
                                <i class="far fa-paper-plane"></i>
                            </div>
                            <i class="far fa-bookmark"></i>
                        </div>
                        <div class="post-info-review">
                            <span class="likes-review">1,234 likes</span>
                            <div class="caption-review">
                                <span class="username-review">amethyst_grl</span>
                                Game day! 🏀
                            </div>
                            <span class="timestamp-review">2 HOURS AGO</span>
                        </div>
                    </div>
                </div>
                

            <!-- Sidebar -->
            <div class="sidebar-review">
                <div class="profile-section-review">
                    <img src="/placeholder-user.jpg" alt="profile" class="profile-pic-review">
                    <div class="profile-info-review">
                        <span class="username-review">eloears</span>
                        <span class="sub-text-review">eloears</span>
                    </div>
                    <button class="switch-btn-review">Switch</button>
                </div>
                <div class="profile-section-review">
                    <img src="/placeholder-user.jpg" alt="profile" class="profile-pic-review">
                    <div class="profile-info-review">
                        <span class="username-review">eloears</span>
                        <span class="sub-text-review">eloears</span>
                    </div>
                    <button class="switch-btn-review">Switch</button>
                </div>
                <div class="profile-section-review">
                    <img src="/placeholder-user.jpg" alt="profile" class="profile-pic-review">
                    <div class="profile-info-review">
                        <span class="username-review">eloears</span>
                        <span class="sub-text-review">eloears</span>
                    </div>
                    <button class="switch-btn-review">Switch</button>
                </div>
                <div class="profile-section-review">
                    <img src="/placeholder-user.jpg" alt="profile" class="profile-pic-review">
                    <div class="profile-info-review">
                        <span class="username-review">eloears</span>
                        <span class="sub-text-review">eloears</span>
                    </div>
                    <button class="switch-btn-review">Switch</button>
                </div>
                <div class="profile-section-review">
                    <img src="/placeholder-user.jpg" alt="profile" class="profile-pic-review">
                    <div class="profile-info-review">
                        <span class="username-review">eloears</span>
                        <span class="sub-text-review">eloears</span>
                    </div>
                    <button class="switch-btn-review">Switch</button>
                </div>
                <div class="profile-section-review">
                    <img src="/placeholder-user.jpg" alt="profile" class="profile-pic-review">
                    <div class="profile-info-review">
                        <span class="username-review">eloears</span>
                        <span class="sub-text-review">eloears</span>
                    </div>
                    <button class="switch-btn-review">Switch</button>
                </div>
                <div class="profile-section-review">
                    <img src="/placeholder-user.jpg" alt="profile" class="profile-pic-review">
                    <div class="profile-info-review">
                        <span class="username-review">eloears</span>
                        <span class="sub-text-review">eloears</span>
                    </div>
                    <button class="switch-btn-review">Switch</button>
                </div>

            </div>
        </div>
    </main>

    <?php
    // Inclusión del footer
    require_once 'Layout/Footer.php';
    ?>
    <script src="Script_Header"></script>
    <script src="Script_Resenas"></script>
</body>
</html>

